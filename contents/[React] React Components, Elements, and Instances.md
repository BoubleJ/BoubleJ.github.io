...
categories: ["React","Components","Elements","DOM","decoupling","Reconciliation","Top-Down Reconciliation"]
summary: "React의 동작원리를 컴포넌트, 엘리먼트으로 분석해봅시다."
...
DOM Tree를 그리는데 필요한 dom node 정보를 가지고 있는데 dom node 정보를 가지고 있기 때문에 리액트에게 화면을 어떻게 그려라~ 라고 전달할 수 있습니다. 
...
위 상술한 class form과 다르게 연결이 끊어져있기 때문에 서로 아무 정보도 공유하지 않아 `decoupling`이 가능합니다. 
...
```
위와 같은 React는 Button(리액트 컴포넌트)가 있다고 가정해봅시다. 리액트는 해당 컴포넌트를 읽으면서 component에게 어떤 엘리먼트를 줄지 물어봅니다.
 
 그리고 그 엘리먼트는 아래 DOM element 입니다. 
 
 리액트 컴포넌트는 이 dom 엘리먼트를 return 받습니다. 
...
리액트는 form 이라는 컴포넌트에게 props를 전달하면서 어떤 엘리먼츠를 줄래? 하면서 물어보고 return element를 요청합니다.  
...
위  Form 함수 호출 시, React 는 Form component 에게 `props를 전달`하며 return element 를 요청합니다.
