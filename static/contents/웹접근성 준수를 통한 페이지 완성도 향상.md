...
때문에 이번 글에서는 저와 비슷한 고민을 하고 계신 분들에게 조금이나마 도움이 되었음 하는 바람과 함께 사내 프로젝트를 진행하며 웹 접근성을 조금 더 꼼꼼하게 반영했던 사례를 공유드려보고자 합니다. UI와 실제 코드를 바탕으로, 어떤 접근성 원칙을 적용했고 이를 코드로 어떻게 구현했는지 정리해보았습니다.

물론 이 글에서 소개하는 코드가 정답이라고 보기는 어렵습니다. 다만 웹 접근성을 준수하기 위해 실제 프로젝트에서 어떤 고민과 시도를 했는지를 공유하는 정도로 가볍게 봐주시면 감사하겠습니다.



# Case1. 탭 메뉴 컴포넌트

아래 이미지는 사내 서비스 메인 화면에 노출되는 탭 메뉴 섹션입니다. 이 UI에서 어떤 웹접근성 원칙을 준수했는지, 실제 코드와 함께 살펴보겠습니다.
...
이렇게 구현해 두면 스크린리더는 “탭 1/3, 기다리면 무료, 선택됨”과 같이 읽어줍니다.

## 3. `role="tabpanel"`과 `aria-labelledby`로 패널과 탭 연결
...
사내 서비스에서는 사이드바 구현을 위해 [shadcn에서 제공하는 Drawer 컴포넌트](https://ui.shadcn.com/docs/components/radix/drawer)를 도입했는데요.
